...
const baseCurso: Cursos = {
  ...
  duracao_semestres: "6 semestres",
  campus: "Campus Brasília",
...
describe("curso mappers", () => {
  it("formats BR prazo and derives aberto/encerrado", () => {
    ...
    expect(
      cursoInscricaoStatus(baseCurso, new Date(2026, 5, 15)),
    ).toBe("aberto");
    expect(
      cursoInscricaoStatus(baseCurso, new Date(2024, 0, 1)),
    ).toBe("encerrado");
  });
  ...
  it("maps Curso → EditalCard", () => {
    const card = cursoToEditalCard(baseCurso, new Date(2026, 5, 15));
...
describe("tipoVagaFromWizard", () => {
  it("maps cotas to TipoVagaCandidatura", () => {
    expect(tipoVagaFromWizard("ppi")).toBe(TipoVagaCandidatura.PII);
    expect(tipoVagaFromWizard("pcd")).toBe(TipoVagaCandidatura.PCD);
    expect(tipoVagaFromWizard("renda")).toBe(TipoVagaCandidatura.ESCOLA_PUBLICA);
