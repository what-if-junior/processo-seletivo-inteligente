  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setOfertas(list);
        const fromUrl = Number(searchParams.get("id_oferta") || "");
        if (Number.isFinite(fromUrl) && fromUrl > 0) {
          setIdOferta(fromUrl);
          try {
            const ch = (await listChamadas(fromUrl)) as ChamadaListItem[];
            if (!cancelled) setChamadas(ch);
          } catch {
            if (!cancelled) setChamadas([]);
          }
        }
      } catch {
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mb-2 text-xs text-slate-500">
              Matriculado = aprovado no processo que confirmou inscrição como aluno no campus.
              Convocados na lista regular ainda sem esta confirmação permanecem como convocados.
            </p>
            <textarea
              value={cpfsText}
...
export default function ChamadasPage() {
  return (
    <Suspense fallback={<p className="text-sm text-slate-500">Carregando chamadas…</p>}>
      <ChamadasPageInner />
    </Suspense>
  );
}
