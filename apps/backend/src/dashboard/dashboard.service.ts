...
/**
 * Convocado = aprovado no processo, ainda sem matrícula no campus.
 * Matriculado is counted separately (never included here).
 */
...
export class DashboardService {
  ...
  async getInsights(filtros: DashboardFiltros): Promise<DashboardInsights> {
    ...
    const cards: DashboardCards = {
      ...
      // efetivação = matriculados / (convocados + matriculados)
      taxa_conversao: percentagem(matriculados, convocados + matriculados),
    };
  ...
  private buildAlerts(table: DashboardLinhaTabela[]): DashboardAlerta[] {
    ...
    for (const linha of table) {
      // convocados already excludes matriculados (approved, not yet campus students)
      if (linha.convocados > 0 && linha.matriculados === 0) {
        alerts.push({
          nivel: 'critico',
          id_oferta: linha.id_oferta,
          mensagem: `${linha.curso} (${linha.campus}): ${linha.convocados} convocado(s) aprovado(s) sem nenhuma matrícula no campus`,
        });
      } else if (linha.convocados > 0) {
        alerts.push({
          nivel: 'aviso',
          id_oferta: linha.id_oferta,
          mensagem: `${linha.curso} (${linha.campus}): ${linha.convocados} convocado(s) ainda sem matrícula no campus`,
        });
      }
